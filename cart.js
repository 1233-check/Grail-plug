...
// Cart stays in localStorage (per-browser). Only checkout talks to Supabase.
...
const GrailCart = {
  ...
  addItem(product) {
    ...
    items.push({
      ...
      price: product.price, size: product.sizeLabel, image: product.image
    });
    this._save(items);
    return true;
  },
  ...
  clear() { this._save([]); },
  getTotal() { return this.getItems().reduce((sum, i) => sum + i.price, 0); },
